...
pkl_path = st.sidebar.text_input(
    "Path to recommender pickle",
    "best_netflix_recommender.pkl"
)
...
try:
    ...
    st.error(f"Could not load pickle file: {e}")
...
if payload:
    ...
    def recommend(title, topn=10, min_rating=0.0):
        ...
        if min_rating > 0:
            result = result[result["IMDB_Rating"].fillna(0) >= min_rating]

        return result.head(topn)[
            ["Series_Title", "Genre", "IMDB_Rating", "Director", "similarity"]
        ]
            ...
            with st.container():
                ...
                with col1:
                    ...
                    if poster_url:
                        ...
                        st.image(
                            f"https://via.placeholder.com/150x220.png?text={row['Series_Title']}",
                            width=200,
                        )

                with col2:
                    st.markdown(
                        f"<h3 style='color:#E50914;'>{row['Series_Title']}</h3>",
                        unsafe_allow_html=True,
                    )
    ...
    st.info("Please provide a valid recommender pickle file in the sidebar.")
